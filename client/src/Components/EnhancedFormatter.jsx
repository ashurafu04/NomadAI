    ...
    lines.forEach((line, index) => {
      ...
      if (line.match(/^\*\*(Day \d+:.+)\*\*$/)) {
        ...
        if (listItems.length > 0) {
          result.push(<ul key={`list-${index}`} className="itinerary-list">{listItems}</ul>);
          listItems = [];
        }
      ...
      // Handle section headers (appears as "* **Section:**")
        ...
        result.push(
          <h4 key={`section-${index}`} className="itinerary-section">{sectionTitle}</h4>
        );
      }
      // Handle list items (appears as "- Item text")
        ...
        listItems.push(
          <li key={`item-${index}`} className="itinerary-item">{processedText}</li>
        );
      }
      // Handle major section titles (appears as "**Title**" alone)
        ...
        const title = line.replace(/^\*\*|\*\*$/g, '');
        result.push(
          <h2 key={`title-${index}`} className="itinerary-title">{title}</h2>
        );
      }
      // Handle empty lines
      else if (line.trim() === '') {
        ...
        result.push(<div key={`space-${index}`} className="itinerary-space"></div>);
      }
      // Handle regular text
      else {
        // Complete any open list
        ...
        result.push(<p key={`text-${index}`} className="itinerary-text">{processedText}</p>);
      }
    });
    
    // Make sure we add any remaining list items
    if (listItems.length > 0) {
      result.push(<ul key="list-final" className="itinerary-list">{listItems}</ul>);
    }
  ...
  const processBoldText = (text) => {
    ...
    return parts.map((part, i) => {
      if (part.startsWith('**') && part.endsWith('**')) {
        ...
        return <strong key={i} className="highlighted-text">{content}</strong>;
      }
      return part;
    });
  };
